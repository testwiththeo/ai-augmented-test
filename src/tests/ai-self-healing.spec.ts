...
test.describe("AI Self-Healing Locators", () => {
  ...
  test("AI suggests alternative selectors from DOM", async ({ page }) => {
    ...
    // Get the DOM snippet
    const domSnippet = await captureDomSnapshot(
      page,
      "email-input",
      "Login email field",
    );
    ...
    if (suggestions.length === 0) {
      console.warn(
        "[Test] AI returned no suggestions (likely rate limited or API unavailable).",
      );
      console.warn(
        "[Test] Add OpenRouter credit or use a paid model for full AI functionality.",
      );
    } else {
  ...
  test("self-heal a broken locator", async ({ page }) => {
    ...
    if (healing?.healed) {
      console.log(
        `[Test] Self-healed! ${healing.originalSelector} → ${healing.healedSelector}`,
      );
    }
